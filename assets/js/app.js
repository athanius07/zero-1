...
function applyFilters(all){
  ...
  let out = all.filter(r=>{
    ...
    if(q){
      const hay = [r['Machine Name'], r['Power Type'], r['OEM'], r['Country of Origin'], r['Class (tons)'], r['Engine / Motor Power'], r['Bucket Size (excavator/wheel loader)'], r['Status'], r['Notes']]
...
(async function(){
  ...
  let sortedKey = null, asc=true;

  const draw = ()=>{
    const rows = current;
    const pageCount = Math.max(1, Math.ceil(rows.length / pageSize));
    if(page>pageCount) page = pageCount;
    renderTablePage(rows, page, pageSize);
    document.getElementById('pageInfo').textContent = `Page ${page} / ${pageCount}`;
  };

  const refilter = ()=>{ current = applyFilters(all); page=1; draw(); };
  ...
  ['chkDump','chkDozer','chkGrader','chkLoader','chkExc','releasedOnly']
    .forEach(id=>document.getElementById(id).addEventListener('change', refilter));
  ...
  document.getElementById('resetBtn').addEventListener('click', ()=>{
    ...
    current = all.slice(); page=1; draw();
  });

  document.getElementById('downloadCsvFiltered').addEventListener('click', ()=>{
    const exportRows = current.length ? current : all;
    if(!exportRows.length){ alert('No data to export.'); return; }
    const normalized = exportRows.map(r=>({...r,'Power Type': normPower(r['Power Type']).map(it=>it.k).join(', ')}));
    const csv = toCSV(normalized);
    const blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'equipment_filtered.csv'; a.click();
  });

  document.getElementById('downloadCsvAll').addEventListener('click', ()=>{
    const normalized = all.map(r=>({...r,'Power Type': normPower(r['Power Type']).map(it=>it.k).join(', ')}));
    const csv = toCSV(normalized);
    const blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'equipment_full.csv'; a.click();
  });

  document.getElementById('prevPage').addEventListener('click', ()=>{ if(page>1){ page--; draw(); }});
  document.getElementById('nextPage').addEventListener('click', ()=>{ page++; draw(); });
  document.getElementById('pageSize').addEventListener('change', (e)=>{ pageSize=parseInt(e.target.value,10); page=1; draw(); });
  ...
  headers.forEach(th=>{
    ...
    th.addEventListener('click', ()=>{
      ...
      current = sortByKey(current, key, thAsc);
      thAsc = !thAsc; page=1; draw();
    });
  });
